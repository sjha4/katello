...
module PulpCertguardClient
  ...
  class PatchedcertguardX509CertGuard < ApiModelBase
    # The unique name.
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PulpCertguardClient::PatchedcertguardX509CertGuard` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PulpCertguardClient::PatchedcertguardX509CertGuard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@ca_certificate.nil? && @ca_certificate.to_s.length < 1
        invalid_properties.push('invalid value for "ca_certificate", the character length must be greater than or equal to 1.')
      end
    ...
    def ca_certificate=(ca_certificate)
      ...
      if ca_certificate.to_s.length < 1
        fail ArgumentError, 'invalid value for "ca_certificate", the character length must be greater than or equal to 1.'
      end
