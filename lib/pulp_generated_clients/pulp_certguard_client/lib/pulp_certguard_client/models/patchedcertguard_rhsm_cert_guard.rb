...
module PulpCertguardClient
  ...
  class PatchedcertguardRHSMCertGuard < ApiModelBase
    # The unique name.
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PulpCertguardClient::PatchedcertguardRHSMCertGuard` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PulpCertguardClient::PatchedcertguardRHSMCertGuard`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
