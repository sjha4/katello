...
module PulpDebClient
  ...
  class DebReleaseArchitecture < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `PulpDebClient::DebReleaseArchitecture` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PulpDebClient::DebReleaseArchitecture`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @architecture.to_s.length < 1
        invalid_properties.push('invalid value for "architecture", the character length must be greater than or equal to 1.')
      end
    ...
    def architecture=(architecture)
      ...
      if architecture.to_s.length < 1
        fail ArgumentError, 'invalid value for "architecture", the character length must be greater than or equal to 1.'
      end
