...
module PulpRpmClient
  class PaginatedrpmModulemdObsoleteResponseList < ApiModelBase
    attr_accessor :count
    ...
    def initialize(attributes = {})
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `PulpRpmClient::PaginatedrpmModulemdObsoleteResponseList`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
