...
import {
  editCVFilter, getCVFilterDetails,
  getFilterRepositories, updateContentView
...
import {
  ...
  selectCVDetails, selectCVFilterRepos, selectCVFilterReposStatus, selectCVFilterReposError, selectCVFilterDetails,
...
const AffectedRepositoryTable = ({ cvId, filterId, repoType, setShowAffectedRepos }) => {
  ...
  const filterDetails = useSelector(state => selectCVFilterDetails(state, cvId, filterId), shallowEqual);
  ...
  const isAddedToFilter = repoId => (repositories.filter(repository => repository.id === repoId).length ? true : false);
...
AffectedRepositoryTable.defaultProps = {
  cvId: '',
  filterId: '',
};
