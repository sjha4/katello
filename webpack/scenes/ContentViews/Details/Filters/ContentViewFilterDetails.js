  ...
  return (
    <Grid hasGutter>
      {loaded && (Object.keys(details).length > 0) ?
        <ContentViewFilterDetailsHeader cvId={cvId} filterId={filterId} details={details} showAffectedRepos={showAffectedRepos} setShowAffectedRepos={setShowAffectedRepos} /> :
        <Loading/>
      }
      <GridItem span={12}>
        <CVFilterDetailType cvId={cvId} filterId={filterId} inclusion={inclusion} type={type} repositories={repositories} showAffectedRepos={showAffectedRepos} setShowAffectedRepos={setShowAffectedRepos}/>
      </GridItem>
    </Grid>
  );
};
